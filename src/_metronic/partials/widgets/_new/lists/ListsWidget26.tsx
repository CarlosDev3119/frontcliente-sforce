...
const rows: Array<{description: string, ref: string}> = [
  {description: 'SFORCE', ref: 'https://smart-force.com/en/'},
  {description: 'Instagram', ref: 'https://www.instagram.com/smartforcemx/'},
]

const ListsWidget26 = ({className}: Props) => (
  <div className={`card card-flush ${className}`}>
    <div className='card-header pt-5'>
      <h3 className='card-title text-gray-800 fw-bold'>Links Externos</h3>
      ...
      {rows.map((row, index) => (
        <Fragment key={`lw26-rows-${index}`}>
          <div className='d-flex flex-stack'>
            <a href={row.ref} className='text-primary fw-semibold fs-6 me-2'>
              {row.description}
            </a>
            <button
              type='button'
